...
#include "TX.h"
...
#if defined(TX__PC)
    #define TX_SetP4PinDriveMode(shift, mode)  \
    do { \
        TX_PC =   (TX_PC & \
                                (uint32)(~(uint32)(TX_DRIVE_MODE_IND_MASK << \
                                (TX_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (TX_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define TX_SetP4PinDriveMode(shift, mode)  \
        do { \
            TX_USBIO_CTRL_REG = (TX_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(TX_DRIVE_MODE_IND_MASK << \
                                    (TX_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (TX_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		TX_SetP4PinDriveMode(TX__0__SHIFT, mode);
    }
#endif
...
void TX_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  TX_INTCFG & (uint32)(~(uint32)position);
    TX_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
